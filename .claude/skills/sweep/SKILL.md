...
5. **If the push is rejected** (e.g., by a hook or commitlint), diagnose the error before retrying:
   - **Commitlint failure** (bad commit message format): This is the ONE case where amend + force-push is allowed. Fix the message with `git commit --amend -m "correct message"` then `git push --force`.

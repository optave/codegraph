...
- **Never run `git checkout <branch>` inside an existing worktree to switch to different work.** A worktree directory can be in use by another session; checking out a different branch there mutates shared state and can yank that session's branch out from under it — this isn't hypothetical, it happened (2026-08-09, cause unattributable). If you need to move a commit to a different branch (e.g. after discovering you're not where you expected), don't check out anything else in the same directory — spin up a fresh worktree instead, per the recovery steps below
- **Recovering a stray commit without touching the contaminated worktree:** `git fetch origin` first, so `origin/<branch>` is current for whichever case below applies — otherwise a branch pushed by someone else since your last fetch still resolves as if it didn't exist. If `<branch>` doesn't exist yet, `git worktree add <new-path> -b <branch> origin/main` — always pass an explicit start point; a bare `-b <branch>` with no third argument branches from whatever HEAD the command is run from, which here is the contaminated worktree's unrelated branch, not a sensible base (plain `<branch>` there always fails once it exists — `fatal: a branch named '<branch>' already exists`). If `<branch>` already exists, use `git worktree add --detach <new-path> <branch>` instead (`origin/<branch>` if `<branch>` exists only on the remote and was never fetched or checked out locally — plain `<branch>` fails there with `fatal: invalid reference: <branch>`) — a detached checkout claims no branch name, so (unlike a plain, non-`-b` `add`) it still works even when `<branch>` is checked out in another worktree right now (a plain `add` fails there with `fatal: '<branch>' is already used by worktree at '<path>'`). Either way, `git cherry-pick <sha>` there; for the detached case, nothing updates `<branch>` automatically, so land it with `git push origin HEAD:<branch>`. Commit objects are shared across all worktrees via the common `.git`, so none of this needs a checkout in the contaminated directory
...
- **Treat your branch as potentially stale before every push, not just before starting work.** Other people and agents share this repo and may have pushed to the same branch (e.g. addressing review feedback while you were also iterating) or moved `origin/main`. Run `git fetch origin` and diff against the remote tracking ref before pushing. If it moved, don't force-push over it — merge the remote changes in (`git merge origin/<branch>`), resolve conflicts by understanding both sides' intent (see the `/resolve` skill), and push a normal merge commit. Prefer merge over rebase on any branch you didn't just create and haven't pushed yet — rebase rewrites commit SHAs, which strands anyone who already pulled or branched from the old history

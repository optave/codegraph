...
<sub>Comparison last verified: August 2026. Claims verified against each repo's README/docs/source. Full analysis: <a href="generated/competitive/COMPETITIVE_ANALYSIS.md">COMPETITIVE_ANALYSIS.md</a></sub>
...
| Hybrid search (BM25 + semantic) | **Yes** | **Yes** | Partial (semantic = paid API) | Keyword only | **Yes** | **Yes** |
...
| Graph export (GraphML / Neo4j / DOT) | **Yes** | **Yes**¹⁴ | **Yes**¹⁵ | — | — | — |
| Open source + commercial use | **Yes** (Apache-2.0) | **Yes** (MIT) | **Yes** (Apache-2.0/MIT) | **Yes** (MIT) | **Yes** (MIT) | Non-commercial¹⁶ |

<sup>¹ colbymchenry/codegraph is an unrelated tool that shares the name. It focuses on reducing AI agent token consumption by pre-indexing code structure for fast context retrieval — not on structural analysis, CI gates, or complexity metrics. ² colbymchenry/codegraph's README headlines "20+ Languages" but its architecture section enumerates 34 names across a tiered native-grammar (20) + fallback-engine pipeline. ³ claude-context indexes 17 file extensions by default (`DEFAULT_SUPPORTED_EXTENSIONS`), but its AST-aware chunker only covers ~10 of them (JS/TS/Python/Java/C/C++/Go/Rust/C#/Scala) — the rest fall back to generic text splitting. ⁴ variable-level vertex model (`arg_in`/`return_out`/`def_use`) with per-function summaries, stitched across call edges for general-purpose use (not just security) across all 34 languages. ⁵ narsil-mcp's `trace_taint`/`get_typed_taint_flow` follow tainted values across function calls for security findings; its general-purpose `get_data_flow`/`get_reaching_definitions` tools are intraprocedural (per-function). ⁶ GitNexus's optional `--pdg` index (TypeScript/JavaScript only) layers taint source→sink tracking on top of its per-function CFG/PDG — scoped to security findings, not general-purpose cross-function variable flow. ⁷ code-review-graph's README (as of its 2026-08-02 benchmark capture) reports its co-change evaluation mode returns 0 predicted files on every graded commit — "not yet a usable measurement." ⁸ narsil-mcp added Merkle-tree incremental indexing and watch mode since our last comparison (previously full re-index). ⁹ colbymchenry/codegraph's OS-native file watcher (FSEvents/inotify/etc.) now reprocesses only changed files with debounced auto-sync — also new since our last comparison; `--force` triggers a full rebuild. ¹⁰ claude-context's `FileSynchronizer` uses a Merkle DAG + per-file MD5 hashing to skip unchanged files, comparable to our own hash-based tiering — though each changed file still round-trips to the embedding provider + vector DB. ¹¹ GitNexus dispatches by cache-key over byte-budgeted chunks and updates per-branch indexes incrementally on checkout — finer-grained than the full-repo reprocess we noted previously, but not per-file hashing. ¹² Codegraph focuses on structural understanding, not vulnerability detection — use dedicated SAST tools (Semgrep, CodeQL, Snyk) for that. ¹³ claude-context requires an external Milvus vector database plus a separate embedding provider before it can index anything. A fully local, keyless path does exist — its FAQ documents self-hosting Milvus via Docker Compose (`MILVUS_ADDRESS=127.0.0.1:19530`) with local Ollama for embeddings (`EMBEDDING_PROVIDER=Ollama`, no API key) — but that is two long-running services to stand up and operate, versus `npm install` with no daemon; the managed path (Zilliz Cloud + an OpenAI/VoyageAI/Gemini key) trades that setup for a hosted dependency and per-token cost. ¹⁴ code-review-graph added GraphML, Neo4j Cypher, Obsidian vault, and SVG export since our last comparison. ¹⁵ narsil-mcp exposes call/import/symbol/CFG graph views through its bundled visualization frontend, plus JSON-LD/N-Quads export for its Code Context Graph. ¹⁶ GitNexus uses the PolyForm Noncommercial 1.0.0 license, with separate commercial/enterprise licensing available.</sup>

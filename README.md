...
<sub>Comparison last verified: August 2026 (full re-verification of every linked repo's live README + GitHub API on 2026-08-09). Full analysis: <a href="generated/competitive/COMPETITIVE_ANALYSIS.md">COMPETITIVE_ANALYSIS.md</a></sub>
...
| Runtime dependencies (direct, all ecosystems)¹⁷ | 50 (4 npm + 46 crates) | **7** | 70 crates¹⁸ | 10 | 23¹⁹ | 36 |
...
Only **4 npm runtime dependencies** — everything else is optional or a devDependency:
...
**The native engine is not dependency-free either.** The prebuilt `@optave/codegraph-{platform}-{arch}` binaries are optional npm packages, so nothing extra lands in your `node_modules` — but they are compiled from 46 direct Cargo crates (mostly tree-sitter grammars). Those are a real supply-chain surface: a vulnerable crate compiled into a binary still needs a rebuild and a release to patch. Counting both ecosystems, codegraph's direct runtime footprint is **50**, not 4.

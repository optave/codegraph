...
<sub>Comparison last verified: August 2026 (full re-verification of every linked repo's live README + GitHub API on 2026-08-09). Full analysis, 45+ additional tools, and methodology notes: <a href="generated/competitive/COMPETITIVE_ANALYSIS.md">COMPETITIVE_ANALYSIS.md</a></sub>

**Not every "code graph" is the same kind of graph.** Before comparing feature-by-feature, it's worth being explicit about what's actually being compared, because this space mixes at least four different products under the same marketing language:

| | Category | What it answers |
|---|---|---|
| 🔧 | **Structural** (codegraph is this) | Graph built directly from parsing/compiling — nodes are real symbols, edges are verified calls/imports/inheritance/dataflow. "What breaks if I change X?" |
| 🧠 | **Knowledge-graph / RAG** | Graph built as a substrate for LLM retrieval or "chat with your codebase" — edges may be semantic or LLM-inferred. "What's related to X?" |
| 🛡️ | **CPG / security** | AST+CFG+PDG combined for vulnerability discovery and taint analysis (Joern lineage). "Where can attacker input reach a dangerous sink?" |
| 🔍📊 | **Retrieval-only / Visualization-only** | Pure embeddings+vector search, or a diagram for humans — no query API. "What looks similar to this?" / "What does this look like?" |

A knowledge-graph tool can tell an agent "these five files are conceptually related to authentication" — useful for orientation — but it generally can't give a complete, deterministic answer to "list every caller of `parseConfig()`." A structural graph does the opposite well and the conceptual-similarity task poorly. They're complementary, not interchangeable, and the table below tags every tool with its category so the comparison stays honest. The deep, feature-by-feature table right below covers codegraph's closest 🔧 structural peers; the [broader landscape](#broader-competitive-landscape) further down covers everything else this document tracks — 45+ additional tools across all four categories, including several notable omissions a reader flagged after the last refresh.

| Capability | codegraph (this repo) | [code-review-graph](https://github.com/tirth8205/code-review-graph) | [narsil-mcp](https://github.com/postrv/narsil-mcp) | [codegraph (other)¹](https://github.com/colbymchenry/codegraph) | [claude-context](https://github.com/zilliztech/claude-context) | [GitNexus](https://github.com/abhigyanpatwari/GitNexus) |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
...
<sup>¹ Unrelated tool sharing the name; focuses on AI token reduction via pre-indexed context, not structural analysis or CI gates. ² README says "20+ languages"; architecture docs list 34 across native + fallback grammar tiers. ³ Indexes 17 file extensions by default; AST-aware chunking covers ~10 — the rest fall back to plain text splitting. ⁴ narsil-mcp's CLI only manages config, lists tools, and serves the visualization frontend — no query commands; all graph queries require an MCP client. ⁵ GitNexus's local HTTP API (`gitnexus serve`, port 4747) only serves its own web UI — no importable library package. ⁶ Variable-level vertex model (`arg_in`/`return_out`/`def_use`) stitched across call edges — general-purpose, not just security, across all 34 languages. ⁷ `trace_taint`/`get_typed_taint_flow` cross function calls for security; general dataflow tools (`get_data_flow`, `get_reaching_definitions`) are per-function only. ⁸ Optional `--pdg` index (TS/JS only) adds taint source→sink tracking — security-scoped, not general cross-function flow. ⁹ Per its own README benchmark (2026-08-02), co-change eval returns 0 predicted files on every graded commit — "not yet a usable measurement." ¹⁰ `get_complexity` covers cyclomatic + cognitive complexity only — no Halstead metrics or maintainability index. ¹¹ Added Merkle-tree incremental indexing + watch mode (previously full re-index). ¹² OS-native file watcher reprocesses only changed files (debounced auto-sync); `--force` forces a full rebuild. ¹³ Merkle DAG + per-file MD5 hashing skips unchanged files, but each changed file still round-trips to the embedding provider + vector DB. ¹⁴ Cache-keyed, byte-budgeted chunk dispatch with incremental per-branch updates on checkout — finer than a full reprocess, but not per-file hashing. ¹⁵ Structural understanding, not vulnerability detection — use dedicated SAST tools (Semgrep, CodeQL, Snyk). ¹⁶ Requires an external Milvus vector DB + an embedding provider. A local keyless path exists (self-hosted Milvus + Ollama) but needs two running services; the managed path (Zilliz Cloud + OpenAI/VoyageAI/Gemini) trades that for cost. ¹⁷ Direct runtime deps declared in each project's own manifest (`package.json`/`pyproject.toml`), excluding dev/optional/transitive deps. ¹⁸ Single compiled binary — no separate packages to install. ¹⁹ `mcp` + `core` packages combined — plus the external vector DB + embedding provider noted above. ²⁰ Added GraphML, Neo4j Cypher, Obsidian vault, and SVG export. ²¹ Visualization frontend graph views, plus JSON-LD/N-Quads export for its Code Context Graph — but not GraphML or Neo4j specifically (a repo-wide code search found zero matches for either string); an internal DOT exporter exists (`to_dot()`) but isn't exposed via CLI or MCP. ²² PolyForm Noncommercial 1.0.0 license; separate commercial/enterprise licensing available.</sup>

### Broader competitive landscape

<sub>140+ tools evaluated as of 2026-08-09 (full detail in the linked analysis); the 45+ most relevant ones are grouped below by graph-model category. Every entry was verified directly against its live GitHub README + API, not carried forward from an earlier pass. Full per-tool detail, scoring, and "vs codegraph" write-ups: <a href="generated/competitive/COMPETITIVE_ANALYSIS.md">COMPETITIVE_ANALYSIS.md</a>.</sub>

> ⚠️ **A note on star counts before you read the tables below:** several tools in this list — marked ⚠️ — show tens of thousands of GitHub stars and thousands of forks accumulated within months of creation, alongside a "subscriber" count (people actually watching the repo) around 0.3–0.4% of their star count. A mature, organically-grown project like Joern sits at roughly 1.1% for comparison. This isn't proof of manipulation, but it's a consistent enough pattern across unrelated repos that we don't treat these star counts as a reliable adoption signal, and neither should you. Full evidence in <a href="generated/competitive/COMPETITIVE_ANALYSIS.md">COMPETITIVE_ANALYSIS.md</a>.

#### 🔧 Structural code graphs (same category as codegraph)

| Tool | Stars | License | Notable for |
|---|---|---|---|
| [getArbor-dev/arbor](https://github.com/getArbor-dev/arbor) | ![](https://img.shields.io/github/stars/getArbor-dev/arbor?style=flat-square&label=%E2%AD%90) | MIT | Closest architectural analog to codegraph here — every edge carries an explicit [0,1] confidence score, native Rust GUI, MCP tools that self-chain via `suggested_next_tool` |
| [DeusData/codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp) ⚠️ | ![](https://img.shields.io/github/stars/DeusData/codebase-memory-mcp?style=flat-square&label=%E2%AD%90) | MIT | Single static C binary, 158 languages, Cypher-like query subset. Its own cited paper reports *lower* answer quality than a naive baseline — see the star-count note above |
| [CodeGraphContext/CodeGraphContext](https://github.com/CodeGraphContext/CodeGraphContext) | ![](https://img.shields.io/github/stars/CodeGraphContext/CodeGraphContext?style=flat-square&label=%E2%AD%90) | MIT | 5 selectable graph-DB backends, pre-indexed `.cgc` bundles for popular repos, 13-IDE MCP setup wizard |
| [SimplyLiz/ckb](https://github.com/SimplyLiz/ckb) | ![](https://img.shields.io/github/stars/SimplyLiz/ckb?style=flat-square&label=%E2%AD%90) | Custom (non-OSI, paid above $25k revenue) | SCIP+LSP+tree-sitter, compound MCP tools, zero-LLM PR review with SARIF output |
| [harshkedia177/axon](https://github.com/harshkedia177/axon) | ![](https://img.shields.io/github/stars/harshkedia177/axon?style=flat-square&label=%E2%AD%90) | Claims MIT, **no LICENSE file** | KuzuDB-backed, Leiden communities, git co-change fused into impact analysis, full WebGL dashboard |
| [cs-au-dk/jelly](https://github.com/cs-au-dk/jelly) | ![](https://img.shields.io/github/stars/cs-au-dk/jelly?style=flat-square&label=%E2%AD%90) | BSD-3-Clause | Academic-grade JS/TS points-to analysis validated against dynamically-traced ground truth, 5 published papers |
| [entrepeneur4lyf/code-graph-mcp](https://github.com/entrepeneur4lyf/code-graph-mcp) | ![](https://img.shields.io/github/stars/entrepeneur4lyf/code-graph-mcp?style=flat-square&label=%E2%AD%90) | MIT | ast-grep-backed, ~29 languages/formats, debounced live file watching |
| [kraklabs/cie](https://github.com/kraklabs/cie) | ![](https://img.shields.io/github/stars/kraklabs/cie?style=flat-square&label=%E2%AD%90) | AGPL-3.0 + commercial | CozoDB/Datalog, interface-dispatch-aware call tracing, HTTP/gRPC endpoint discovery |
| [MikeRecognex/mcp-codebase-index](https://github.com/MikeRecognex/mcp-codebase-index) | ![](https://img.shields.io/github/stars/MikeRecognex/mcp-codebase-index?style=flat-square&label=%E2%AD%90) | AGPL-3.0 + commercial | Git-diff-triggered incremental reindex in ~1-2ms, zero runtime deps |
| [NeuralRays/codexray](https://github.com/NeuralRays/codexray) | ![](https://img.shields.io/github/stars/NeuralRays/codexray?style=flat-square&label=%E2%AD%90) | MIT | TF-IDF "semantic" search instead of a downloaded embedding model (~50MB vs ~500MB) |
| [Technologicat/pyan](https://github.com/Technologicat/pyan) | ![](https://img.shields.io/github/stars/Technologicat/pyan?style=flat-square&label=%E2%AD%90) | GPL-2.0 | Python-only, MRO-aware `super()` resolution, PEP 695/walrus/`match` support |
| [Adrninistrator/java-all-call-graph](https://github.com/Adrninistrator/java-all-call-graph) | ![](https://img.shields.io/github/stars/Adrninistrator/java-all-call-graph?style=flat-square&label=%E2%AD%90) | Apache-2.0 | Bytecode-level (not source) Java call graphs, Spring/MyBatis-aware, jar-diff blast radius |
| [Vasu014/loregrep](https://github.com/Vasu014/loregrep) | ![](https://img.shields.io/github/stars/Vasu014/loregrep?style=flat-square&label=%E2%AD%90) | MIT OR Apache-2.0 | Cache lives outside the analyzed repo; identical API as Rust crate, Python wheel, CLI, and Claude Code Skill (not MCP) |
| [JohT/code-graph-analysis-pipeline](https://github.com/JohT/code-graph-analysis-pipeline) | ![](https://img.shields.io/github/stars/JohT/code-graph-analysis-pipeline?style=flat-square&label=%E2%AD%90) | GPL-3.0 | jQAssistant+Neo4j, 200+ CSV/Cypher reports, ML-based anomaly detection with SHAP — built for human analysts/CI, not agents |
| [Graphify-Labs/graphify](https://github.com/Graphify-Labs/graphify) ⚠️ | ![](https://img.shields.io/github/stars/Graphify-Labs/graphify?style=flat-square&label=%E2%AD%90) | Apache-2.0 | YC-backed. One graph spans code *and* docs/PDFs/schemas with explicit extracted-vs-inferred edge provenance; git-aware merge driver |
| [zzet/gortex](https://github.com/zzet/gortex) | ![](https://img.shields.io/github/stars/zzet/gortex?style=flat-square&label=%E2%AD%90) | Apache-2.0 | 100+ MCP tools, cross-repo HTTP/RPC contract matching, heavy supply-chain hardening (Sigstore/SLSA L3) |
| [repowise-dev/repowise](https://github.com/repowise-dev/repowise) | ![](https://img.shields.io/github/stars/repowise-dev/repowise?style=flat-square&label=%E2%AD%90) | AGPL-3.0 | Zero-LLM deterministic core; publishes head-to-head benchmarks against competitors *and explicitly states which rows it loses* |
| [ozgurcd/gograph](https://github.com/ozgurcd/gograph) | ![](https://img.shields.io/github/stars/ozgurcd/gograph?style=flat-square&label=%E2%AD%90) | MIT | Go-only by design, trades breadth for CHA/SSA-enriched compiler-level resolution |
| [kuberstar/qartez-mcp](https://github.com/kuberstar/qartez-mcp) | ![](https://img.shields.io/github/stars/kuberstar/qartez-mcp?style=flat-square&label=%E2%AD%90) | Custom (non-OSI) | A guard hook flags risky AI-driven edits before they land — a safety angle not seen elsewhere in this list |
| [codegraph-ai/CodeGraph](https://github.com/codegraph-ai/CodeGraph) | ![](https://img.shields.io/github/stars/codegraph-ai/CodeGraph?style=flat-square&label=%E2%AD%90) | Apache-2.0 | Yet another same-concept "CodeGraph" — one engine shared by a VS Code extension *and* a full JetBrains plugin |

#### 🧠 Knowledge-graph / RAG tools (different category — see the framing above)

| Tool | Stars | License | Notable for |
|---|---|---|---|
| [vitali87/code-graph-rag](https://github.com/vitali87/code-graph-rag) | ![](https://img.shields.io/github/stars/vitali87/code-graph-rag?style=flat-square&label=%E2%AD%90) | MIT | Own tagline: "the ultimate RAG for your monorepo" — NL-to-Cypher over Memgraph+Qdrant. Requires Docker+cmake+ripgrep despite the simple-looking `pip install` |
| [Jakedismo/codegraph-rust](https://github.com/Jakedismo/codegraph-rust) | ![](https://img.shields.io/github/stars/Jakedismo/codegraph-rust?style=flat-square&label=%E2%AD%90) | Claims MIT/Apache, **no LICENSE file** | "100% Rust implementation of code graphRAG" — 4 agentic (LATS/ReAct/Reflexion) MCP tools over SurrealDB |
| [nahisaho/CodeGraphMCPServer](https://github.com/nahisaho/CodeGraphMCPServer) | ![](https://img.shields.io/github/stars/nahisaho/CodeGraphMCPServer?style=flat-square&label=%E2%AD%90) | MIT | Explicit Microsoft-GraphRAG-modeled implementation. **Exposes an `execute_shell_command` MCP tool** — arbitrary shell execution for any connected agent, worth knowing before installing |
| [Durafen/Claude-code-memory](https://github.com/Durafen/Claude-code-memory) | ![](https://img.shields.io/github/stars/Durafen/Claude-code-memory?style=flat-square&label=%E2%AD%90) | None stated | Qdrant-backed semantic memory with a pre-write hook that blocks likely-broken edits |
| [diffbot/mcp-code-graph](https://github.com/diffbot/mcp-code-graph) | ![](https://img.shields.io/github/stars/diffbot/mcp-code-graph?style=flat-square&label=%E2%AD%90) | MIT | Thin client for CodeGPT/DeepGraph's proprietary cloud graph — no local/offline option, requires an account for private repos |
| [Egonex-AI/Understand-Anything](https://github.com/Egonex-AI/Understand-Anything) ⚠️ | ![](https://img.shields.io/github/stars/Egonex-AI/Understand-Anything?style=flat-square&label=%E2%AD%90) | MIT | Multi-agent LLM pipeline (not tree-sitter-based) that writes plain-English guided tours of a codebase |
| [er77/code-graph-rag-mcp](https://github.com/er77/code-graph-rag-mcp) | ![](https://img.shields.io/github/stars/er77/code-graph-rag-mcp?style=flat-square&label=%E2%AD%90) | MIT | **Declared obsolete and canceled by its own README** — listed for completeness, not as an active option |

#### 🛡️ CPG / security-focused tools (different category — see the framing above)

| Tool | Stars | License | Notable for |
|---|---|---|---|
| [joernio/joern](https://github.com/joernio/joern) | ![](https://img.shields.io/github/stars/joernio/joern?style=flat-square&label=%E2%AD%90) | Apache-2.0 | The category's origin point — full CPG (AST+CFG+PDG), Scala query DSL, analyzes source/bytecode/binaries |
| [Fraunhofer-AISEC/cpg](https://github.com/Fraunhofer-AISEC/cpg) | ![](https://img.shields.io/github/stars/Fraunhofer-AISEC/cpg?style=flat-square&label=%E2%AD%90) | Apache-2.0 | LLVM-IR frontend extends coverage to any LLVM-targeting language; optional MCP+AI-chat module |
| [ShiftLeftSecurity/codepropertygraph](https://github.com/ShiftLeftSecurity/codepropertygraph) | ![](https://img.shields.io/github/stars/ShiftLeftSecurity/codepropertygraph?style=flat-square&label=%E2%AD%90) | Apache-2.0 | The formal CPG specification Joern is built on — a JVM library, not an end-user tool (its own README points you to Joern) |
| [Lekssays/codebadger](https://github.com/Lekssays/codebadger) | ![](https://img.shields.io/github/stars/Lekssays/codebadger?style=flat-square&label=%E2%AD%90) | GPL-3.0 | Joern-CPG-based MCP server, 10 CWE-mapped detectors, ICSE 2026 companion paper |
| [clouditor/cloud-property-graph](https://github.com/clouditor/cloud-property-graph) | ![](https://img.shields.io/github/stars/clouditor/cloud-property-graph?style=flat-square&label=%E2%AD%90) | Apache-2.0 | Extends CPG analysis into cloud runtime security assessment. Its own README: "please do not expect API stability" |

#### 🔍 Retrieval-only & 📊 visualization-only (no call graph at all)

| Tool | Stars | License | Notable for |
|---|---|---|---|
| [FarhanAliRaza/claude-context-local](https://github.com/FarhanAliRaza/claude-context-local) | ![](https://img.shields.io/github/stars/FarhanAliRaza/claude-context-local?style=flat-square&label=%E2%AD%90) | None stated | 100% local semantic search via Google's EmbeddingGemma + sharded FAISS |
| [anrgct/autodev-codebase](https://github.com/anrgct/autodev-codebase) | ![](https://img.shields.io/github/stars/anrgct/autodev-codebase?style=flat-square&label=%E2%AD%90) | Claims MIT, **no LICENSE file** | Qdrant-backed vector search is the headline; a separate 7-language structural call-graph command exists too |
| [seatedro/glimpse](https://github.com/seatedro/glimpse) | ![](https://img.shields.io/github/stars/seatedro/glimpse?style=flat-square&label=%E2%AD%90) | MIT | Primarily a clipboard/context-dump tool for LLMs; a tree-sitter call-graph feature is a secondary add-on. Dormant since Jan 2026 |
| [dundalek/stratify](https://github.com/dundalek/stratify) | ![](https://img.shields.io/github/stars/dundalek/stratify?style=flat-square&label=%E2%AD%90) | MIT | Renders hierarchical DGML/3D dependency diagrams for external viewers (Visual Studio, CodeCharta) |
| [DucPhamNgoc08/CodeVisualizer](https://github.com/DucPhamNgoc08/CodeVisualizer) | ![](https://img.shields.io/github/stars/DucPhamNgoc08/CodeVisualizer?style=flat-square&label=%E2%AD%90) | MIT | VS Code extension rendering Mermaid flowcharts + dependency diagrams — no CLI, no MCP |
| [xnuinside/codegraph](https://github.com/xnuinside/codegraph) | ![](https://img.shields.io/github/stars/xnuinside/codegraph?style=flat-square&label=%E2%AD%90) | MIT | Python-only interactive D3.js HTML diagram — no query API, needs no code execution to analyze |
| [helabenkhalfallah/code-health-meter](https://github.com/helabenkhalfallah/code-health-meter) | ![](https://img.shields.io/github/stars/helabenkhalfallah/code-health-meter?style=flat-square&label=%E2%AD%90) | MIT | Six-dimension code-health score (MI, Halstead, Louvain modularity), peer-reviewed in ACM TOSEM 2025. No agent/MCP integration |

#### Adjacent tools you may already have installed

Not scored above because they don't build or persist a code graph at all — but they're extremely popular and frequently used alongside a tool like codegraph, so it's worth being explicit about how they differ.

| Tool | Stars | What it actually is |
|---|---|---|
| [semgrep/semgrep](https://github.com/semgrep/semgrep) | ![](https://img.shields.io/github/stars/semgrep/semgrep?style=flat-square&label=%E2%AD%90) | Per-file AST pattern matching for SAST — ships an official MCP server, but the open-source edition is limited to single-function/file analysis (cross-file dataflow needs the paid Pro engine) |
| [ast-grep/ast-grep](https://github.com/ast-grep/ast-grep) | ![](https://img.shields.io/github/stars/ast-grep/ast-grep?style=flat-square&label=%E2%AD%90) | Structural search/lint/codemod tool — answers "find this syntax pattern," not "who calls this function"; complements a dependency graph rather than replacing one |
| [universal-ctags/ctags](https://github.com/universal-ctags/ctags) | ![](https://img.shields.io/github/stars/universal-ctags/ctags?style=flat-square&label=%E2%AD%90) | Flat symbol index (name/kind/file/line) with no edges between symbols at all — answers "where is X defined," never "who calls X" |
| [github/codeql](https://github.com/github/codeql) | ![](https://img.shields.io/github/stars/github/codeql?style=flat-square&label=%E2%AD%90) | Query-based vulnerability finder. The queries/libraries here are MIT, but the CLI/engine that runs them (`github/codeql-cli-binaries`) is proprietary-with-narrow-exceptions and blocks routine use on closed-source code without a commercial contract |

Also notable but not realistically usable by an agent today without significant integration work: [Kythe](https://github.com/kythe/kythe) (Google's cross-language code-indexing platform) and [Glean](https://github.com/facebookincubator/Glean) (Meta's equivalent) both build genuinely precise, compiler-derived cross-reference graphs — the canonical examples of "structural graph at hyperscale" — but neither ships an MCP server or an agent-friendly CLI; both require standing up a compilation-extraction pipeline. [SCIP](https://github.com/scip-code/scip) (successor to LSIF, now independently governed) is the index *format* several of the tools above build on, not a tool itself. [stack-graphs](https://github.com/github/stack-graphs), GitHub's own name-resolution library, was discontinued in September 2025.

...
**Largest files:** `dataflow.ts` (701 LOC), `structure.ts` (694 LOC), `cfg.ts` (579 LOC), `complexity.ts` (557 LOC). None exceed the 700 LOC threshold — disciplined.
...
The TypeScript migration is complete (280 .ts files, 0 .js in src/). The CLAUDE.md still says "JS source is plain JavaScript (ES modules)" — this is stale documentation.
...
- CLAUDE.md references "JS source is plain JavaScript" — stale after TypeScript migration
...
| **Max cognitive complexity** | 243 (`walk_node_depth` in JS extractor) |
| **Avg maintainability index** | 60.1 |
| **Functions above threshold** | 413 (of 1,769) |
| **Production dependencies** | 3 |
| **Dead code (total)** | 8,285 symbols |
...
The raw "8,960 dead" count from `codegraph stats` is misleading. Breaking down:
...
2. **Stale CLAUDE.md references** — "JS source is plain JavaScript (ES modules)" is now false. Update to reflect TypeScript.

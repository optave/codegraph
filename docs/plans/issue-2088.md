 ...
 *
 * Reuses `functionScopeDeclaresVar`'s own traversal rule one-for-one — skip
 * any child in `FUNCTION_SCOPE_NODE_TYPES`, since a nested function opens
 * its own, independent `var`/Annex-B scope that this walk must not cross
 * into — generalised from that function's boolean "does at least one exist"
 * return to a count, because `findTopLevelFunctionNodeByName`'s own
 * fail-safe needs to know not just THAT an additional hoisted declaration
 * exists, but how many exist in total once combined with the direct-children
 * count gathered above.
 *

...
> `FUNCTION_SCOPE_NODE_TYPES`-ancestor walk rounds 19/20 already used
> (`findEnclosingFunctionBody`, unchanged) — NOT `findDeclaringScopeNode`.**
...
>   Verified by ablation, not merely argued: reintroducing round 18's own
>   kind-gated logic in place of the uniform recomputation, with every other
>   fix in this round held fixed, flips nine of this round's own new
>   regression fixtures (the simplest trigger and eight of its nine
>   manifestations) from correct back to wrong — reuse-unchanged-for-`let`/
>   `const` is precisely the bug this round closes, and the gate's very
>   purpose was to decide when to depart from that reuse. Removing the gate
>   and the machinery it guarded — the bespoke "is this declarator `var`-kind"
>   check and its own widening branch, `isVarKindDeclarator` — deletes
>   machinery outright, per the question's own framing.
...
> cases (bk)–(bq) (WU-10, below) are the executed blocking-finding repro, its
> nine manifestations (consolidated per this section's own established
> "does this exercise a different code path" discipline — see each new
> case's own commentary for which manifestations it stands in for), and A10.
...
//      (`findEnclosingFunctionBody`) rather than `findDeclaringScopeNode` —
...
//      (`findEnclosingFunctionBody`, per #2643 in the round-21 essay), which

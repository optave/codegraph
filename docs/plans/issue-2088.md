...
- **Blocks:** WU-10
- **Files:** `crates/codegraph-core/src/domain/graph/builder/stages/build_edges.rs`, `crates/codegraph-core/src/domain/graph/builder/stages/import_edges.rs`, `crates/codegraph-core/src/db/connection.rs`, `src/domain/graph/builder/stages/native-orchestrator.ts`
- **Input contract:** `FileEdgeInput` carrying `object_literal_sites`; the two new tables
- **Output contract:** identical edges and identical `roles` output to the WASM path
- **Verification:** `cargo test -p codegraph-core` then `cargo clippy --all-targets -- -D warnings`
...
- `native-orchestrator.ts` — `objectLiteralSites` threaded into the `FileEdgeInput` NAPI payload for WASM-parsed files, exactly as `computedDispatchTableEvidence` already is.

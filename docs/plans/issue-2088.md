...
>   correction, only the count was false. reuse-unchanged-for-`let`/`const` is
>   precisely the bug this round closes, and the gate's very purpose was to
>   decide when to depart from that reuse. Removing the gate and the
>   machinery it guarded — the bespoke "is this declarator `var`-kind" check
...
> (bw), (bx), (by)) and TWO deliberate controls, (br) (the `let`-vs-`var`
...
//      (named `findEnclosingFunctionBody` by round 22, for exposition; per
//      #2643 in the round-21 essay for why this walk, rather than
...
// (by) (ROUND 22, #2088, blocking) The identical trigger placed one hop into
//      this design's own FOR-OF-LOOP-VARIABLE recursion: the decoy targets
//      the loop variable's name (`r86`), not the table's, in a function
//      reached only during `r86`'s own recursive reference search -- the
//      for-of counterpart of case (bx), one recursion branch over.
function fnAlpha86() { return 1; }
function sink86(x) { return x.alpha(); }
const T86 = { alpha: fnAlpha86 };
function go86() {
  for (const r86 of [T86]) {
    function inner86() {
...
- **container** — module (top level), `function_declaration`, arrow function, IIFE, `async function`, generator function, object method, class method, class STATIC BLOCK (added this round, motivated directly by (bq)–(by) above), callback (passed as an argument to another function), bare block, `if` block, `try` block, `switch` case, loop body (`for`/`while`/`for-of`/`for-in`).
- **decoy** — none (control), `var`, `let`, `const`, `function`, `class`, a parameter, a `catch` parameter, an import, a for-of loop head, and a **var declared inside a nested `class ... { static { ... } } `** (added this round — the exact shape (bq)–(by) exercise; this is what makes the matrix itself, not merely this round's own nine hand-written cases, the thing that would have caught this round's own bug had it existed before now).
...
- **owner form** — `const`/`let`/`var` direct declarator, an array element (`const arr = [N]`), a factory return (`function make() { return N; } const x = make();`).

Fifteen containers × eleven decoys × eleven escape shapes × three owner forms is 5,445 nominal combinations. Not all are meaningful or even syntactically constructible — a `catch` parameter decoy needs a `try`/`catch` wrapper the container axis may already provide redundantly with itself, an `import` decoy needs a second file, and a `class static block` CONTAINER (as opposed to decoy) changes what "escape shape" even means inside it (a static block has no `this` bound to an instance) — the generator must SKIP a combination it cannot construct meaningfully rather than force one, and must record which combinations it skipped and why, so a shrinking skip-count over time is itself a signal the axes are maturing, not silently reducing coverage.

**Contract, mechanical and non-negotiable:** for every combination the generator DOES construct, it produces one small JS file, executes it under plain Node to obtain the RUNTIME ORACLE (does the shape's own `fnA`-equivalent actually get invoked — the same ground truth every hand-written case above already asserts by inspection, here obtained by execution instead), separately computes this design's own `escapes` verdict for the same file (once WU-2 lands, via the real extractor; a reference port of `computeObjectLiteralSiteEscapes`'s pseudocode may stand in for it before then, exactly as this round's own ablation harness did for `functionScopeDeclaresVar`, above), and asserts the two AGREE. A mismatch is a new, mechanically-found gap — filed the same way every hand-found one in this document has been, not silently absorbed into the matrix's own expected-mismatch list. This is the same discipline the ablation standing rule (round 19, Success Criteria) already requires per-fixture, applied here at the scale of the whole cross-product instead of one case at a time.

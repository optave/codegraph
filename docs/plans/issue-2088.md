...
>
> **A further, EXECUTED finding, not merely hypothesized: the divergence is not only a miss, it is a bounded, pre-existing COLLISION risk any bare/normalized fallback inherits.** Two different TS classes, `ClassA`/`ClassB`, each with their own `run` instance method and their own for-of over a DIFFERENT array bound to `r`, both collapse onto the SAME bare pts key `run::r` — confirmed with the real pipeline: `pts.get('run::r')` holds the UNION of both classes' array-element tokens, while `findCaller` itself still correctly and independently returns `ClassA.run`/`ClassB.run` for the two calls (only the extractor's *pts key* is collapsed, not `findCaller`'s own resolution). This is bounded to over-crediting an unrelated same-named method's array contents as a possible target — never to hiding a real one — and it is not a new risk this fix introduces: it is already latent in root cause (a) today for every OTHER `enclosingFunc`-scoped pts consumer in the file (param-flow, array-elem, spread-arg, array-callback constraints), independently of #2088 or B1.
>

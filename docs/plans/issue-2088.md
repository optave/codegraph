...
**Owner placement, pinned down** (the pre-round-22 spec never said this, and the two readings differ by 2,505 constructed cases): the **container** axis places the DECOY and the escape shape's own reference to the owner (direct, or through the alias/hop-depth axis above) — never the object literal itself, which this matrix always declares at MODULE scope. Varying the owner's OWN declaration depth (module/function/block) is the separate, already-established round-8/round-10 scope-coverage trio discipline (Testing Strategy section, above); this matrix does not re-cross it combinatorially, both because the hand-written trios that discipline already produced cover it and because crossing it here would multiply every other axis by a further factor of three for no coverage those trios do not already provide.

Fifteen containers × fourteen decoys (eleven, minus the for-of loop head's now-single value, plus its four kind variants) × eleven escape shapes × five owner forms (three, minus the collapsed direct declarator, plus its three keyword variants) × six alias/hop-depth values × twelve content values is 831,600 nominal combinations — up from the pre-round-22 5,445 by the same generator discipline this sentence already relied on at the smaller number: not all are meaningful or even syntactically constructible — a `catch` parameter decoy needs a `try`/`catch` wrapper the container axis may already provide redundantly with itself, an `import` decoy needs a second file, and a `class static block` CONTAINER (as opposed to decoy) changes what "escape shape" even means inside it (a static block has no `this` bound to an instance) — the generator must SKIP a combination it cannot construct meaningfully rather than force one, and must record which combinations it skipped and why, so a shrinking skip-count over time is itself a signal the axes are maturing, not silently reducing coverage.

**Execution is the detector for unconstructible combinations — there is no separate, hand-written rule for predicting them, and the plan must not attempt to write one.** Every faithful attempt at such a rule has still leaked SyntaxErrors that only running the parser caught — 36 and 231 of them, in two independently-built implementations of this same spec, both written in good faith against the "skip what you cannot construct" instruction above and both still wrong about which combinations that was. The generator instead attempts to write out and parse every nominal combination unconditionally; a `SyntaxError` — from Node, from tree-sitter, or from both — IS the skip signal: record the combination and the error, then move on. A `SyntaxError` means skip-and-record, never fail — it must never fail the harness or the CI run itself, and a combination skipped this way counts toward the same shrinking skip-count the paragraph above already tracks.

**Coverage check: the axes above, together, generate all five of this design's own historical blocking findings that a hand-written case first caught — proof the matrix would now have found each mechanically, not merely a claim that it should.**
- `const u = T` (round 21): container = `function_declaration`, alias/hop-depth = `const` alias (one hop), decoy = none, content = identifier value, owner form = `const` direct, escape shape = `sink(N)` call.
- `class_static_block` (round 22): container = any function-shape value, decoy = var-inside-`class_static_block`, alias/hop-depth = direct, content = identifier value, owner form = `const` direct, escape shape = `sink(N)` call.
- `__proto__` (round 19 finding 1): content = non-computed `__proto__` pair, which escapes unconditionally per condition 4 regardless of every other axis — the minimal generated case (container = module, decoy = none, alias/hop-depth = direct, owner form = `const` direct) already exposes it.
- a `var`-kind for-of head (round 17 finding 1): decoy = for-of loop head, kind = `var`, owner form = `var` direct (the REBOUND top-level owner), container = any function-shape value the decoy sits inside.
- a bare `run => {}` arrow parameter (round 12): container = arrow function, decoy = a parameter, realized (per the decoy axis's own note above) as the bare, unparenthesized single-identifier form specifically for this container.

Each is a single generated combination reachable once its named axis/value exists — none requires an axis beyond the six above.

**Contract, mechanical and non-negotiable, and a THREE-TERM predicate — never the two-way `invoked ⟺ !escapes` check a prior draft of this contract implied.** For every combination the generator DOES construct, it produces one small JS file and executes it under plain Node to obtain the RUNTIME ORACLE: does the shape's own `fnA`-equivalent actually get invoked (`invoked`) — the same ground truth every hand-written case above already asserts by inspection, here obtained by execution instead. It separately computes this design's own verdict for the same file (once WU-2 lands, via the real extractor; a reference port of `computeObjectLiteralSiteEscapes`'s pseudocode, reporting both the `escapes` bit AND whether its own walk located a genuine, tracked reference constituting the invoking evidence, may stand in for it before then, exactly as this round's own ablation harness did for `functionScopeDeclaresVar`, above) — `escapes`, and `hasT1Evidence` (true iff the walk identified a real, tracked reference as the invocation, as opposed to a vacuously-narrowed walk — round 8's own headline bug and this round's own `class_static_block` bug are exactly this shape — finding none at all despite `escapes` reading `0`).

A two-way `invoked ⟺ !escapes` reading endorses exactly the class of bug this plan exists to catch, and flags exactly the fix that closes it. Executed on the matrix's own generated (bq)-equivalent (`function go() { class C { static { var T = 1; } } sink(T); } go();`): `invoked = true` under both the buggy and the fixed engine; the BUGGY engine's `escapes = 0` (wrong — the walk pruned away the very subtree containing the genuine invocation) reads as "agreement" under a two-way check and PASSES, while the FIXED engine's correct `escapes = 1` reads as "disagreement" and is wrongly flagged as a NEW gap — the matrix endorses the bug and flags the fix, exactly backwards. Measured false-alarm rate against this repo's own fixtures: 43% under one reading of the prior wording, 72% under the other, both benign, neither acceptable. The mismatch predicate this contract actually asserts is:

> `invoked === true && escapes === 0 && !hasT1Evidence`

...
function findTopLevelFunctionNodeByName(root: TreeSitterNode, name: string): TreeSitterNode | null {
  ...
  let declarationCount = 0; // ROUND 14 (#2088 finding 2) — see doc comment below.
  ...
  // name. Fail safe instead — see this function's own doc comment.
...
**Naming convention:** the correlation test's seven shapes (used by their numbers, 1–7, throughout this plan) and the escape-fallback test's shapes ((a)–(z), then, round 11, (aa)–(ac), then, round 12, (ad), then, round 13, (ae), then, round 14, (af)–(ai) — spreadsheet-style continuation rather than renumbering, so every existing cross-reference to a lettered case stays valid — used by their letters) are two independent, alphabetically/numerically-keyed lists — a re-verified shape 2 above is unrelated to the lettered cases below.
...
- [ ] **(round 14, #2088 finding 2 — the single-declaration contract).** `findTopLevelFunctionNodeByName` never returns a top-level declaration of `name` with confidence when a SECOND (or later) top-level declaration of the identical name also exists — enforced structurally by counting every top-level declaration encountered (a `function_declaration` or a `variable_declarator` match, regardless of its own value's shape) and returning `null` once more than one is found, not merely by returning whichever declaration the scan happens to reach first. Verified by WU-10 escape-fallback cases (ah) (a `var` redeclared to a `this`-using function must escape) and (ai) (the same shape via a duplicate sibling `function` declaration must escape too) — neither may silently resolve to the FIRST declaration's own, superseded value.

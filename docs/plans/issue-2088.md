...
> the nearest enclosing `FUNCTION_SCOPE_NODE_TYPES` member starting from the
> `for_in_statement` node itself, or `root` if none — and passes THAT as
> `declaringScope`, instead of reusing the outer call's. A `let`/`const`
...
// 23. (ROUND 19) A COMPUTED `__proto__` key — proves the new non-computed-
//     `__proto__`-key check in `literalHasUnmodeledThisReference` is scoped
//     to the special, non-computed spelling ECMA-262 Annex B.3.1 actually
//     gives magic meaning to, not to every pair whose key merely SPELLS
//     `__proto__`: `['__proto__']: { … }` creates an ORDINARY own property,
//     never touches `[[Prototype]]`, and must stay non-escaping exactly
//     like any other object-valued pair.
function fnAlpha62() { return 1; }
function run62() { return 2; }                        // plain method value, this-free
const T62 = {
  alpha: fnAlpha62,
  run62,
  ['__proto__']: { go() { return 'unrelated'; } },    // COMPUTED — an ordinary own key, no prototype magic
};
T62.run62();
// EXPECT: fnAlpha62 dead (nothing invokes `.alpha(`); escapes === 0 for
// T62's site — the computed spelling does not trip the round-19 check.
...
// 25. (ROUND 19) A `let`-kind for-of loop variable, with an UNRELATED,
//     later, wider-scoped binding of the identical name — proves the new
//     `var`-kind boundary-widening in the for-of recursion (round 19) is
//     gated on `kind === 'var'` specifically, not applied unconditionally:
//     a `let`-kind loop variable keeps its un-widened, block-scoped
//     boundary, so the walk for `r64` never reaches — and is never
//     confused by — the unrelated, sibling `r64` declared outside the
//     loop's own block.
...
- **Classic-script `globalThis.T.alpha()` READS are invisible to `allReferencesTracked`'s reference walk** — round 16 (#2634) already accepts the classic-script premise for a `globalThis`-qualified **write** to a script-scope `var`; the symmetric READ was found during round 19's review and is **not** modeled: a `member_expression`'s `property` is a `property_identifier`, a node kind round 19's own widened filter deliberately excludes (correlation shape 24 depends on that exclusion), so `globalThis.T.alpha()` is invisible in both directions. **UNDER-escape in direction, and — unlike #2610, immediately above — NOT a pre-existing gap this plan inherits and verifiably does not worsen: `allReferencesTracked` is new machinery THIS plan introduces, and no other condition (condition 2's export check does not apply to a non-module classic script at all) already bounds the cost the way #2610's own argument requires.** Per this plan's own DIRECTION-labels standing rule (Success Criteria, below), an UNDER-escape gap "may never be filed as an accepted limitation on either list... it must be fixed in the round that finds it, or the plan does not ship" — this bullet is filed anyway, at explicit orchestrator direction for this round rather than fixed inline, and is recorded here as a **flagged departure from that rule, not a quiet exception to it**: closing it requires teaching the reference walk a `globalThis`-qualified read shape symmetric to `isGlobalObjectQualifiedWrite`'s existing write-side one, in BOTH engines, with its own WU-10 fixture pair — real design scope for its own round, not a drive-by fix. A human reviewer should confirm this departure is acceptable before the round ships, or require it closed first. → issue **#2640**

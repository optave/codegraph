...
function subtreeContainsReassignmentOf(node: TreeSitterNode, name: string, depth: number): boolean {
  ...
  if (node.type === 'assignment_expression' || node.type === 'augmented_assignment_expression') {
    ...
    const left = node.childForFieldName('left');
    if (left && patternBindsName(unwrapParens(left), name) && !node.childForFieldName('kind')) return true;
  }
  for (let i = 0; i < node.childCount; i++) {
    const child = node.child(i);
...
**The correlation test** covers the twelve shapes the design claims (three from earlier rounds, two added in round 8 to close the testing blind spot described below, two added in round 9 for finding 1's over-escape check and finding 2's pass-ordering check, one added in round 15 to prove the var/Annex-B hoist-count fix's own `let`/`const` exclusion, and four added in round 16 — one guard per that round's own four fixes, proving none of them over-escapes the legitimate neighbouring shape), each asserted under **both** engines (`--engine wasm` and `--engine native`, skipped with an explicit message rather than silently if `isNativeAvailable()` is false — never a silent skip):
...
// 9. (ROUND 16) An ordinary, harmless `parenthesized_expression` elsewhere
//    in the file, not used as a write or loop-head target at all — proves
//    `unwrapParens` (new, round 16) is reached only through the two
//    write-scan/shadow call sites' own `left`-field reads and does not
//    misfire against an unrelated grouped expression merely because one
//    exists in the same file as a correlated table.

...
//     would have passed it identically.** This shape replaces it, correctly
//     this time: `run50` is the table's own handler, and the for-loop's
//     init clause carries an UNRELATED, differently-named malformed `using`
//     attempt (`other50`) — since the fix is unconditional (it does not,
//     and structurally cannot, resolve a name out of an `ERROR` node), the
//     table still escapes here too, exactly as it would if the malformed
//     clause used `run50`'s own name instead.
function mk50() { return { [Symbol.dispose]() {} }; }
function fnAlpha50() { return 1; }
function run50() { return 0; }                      // module-level, this-free
function install50() {
  const disposable50 = function () { return this.alpha(); };
  disposable50[Symbol.dispose] = () => {};
  for (using other50 = mk50(); true; ) {              // UNRELATED name, malformed init
    const T50 = { alpha: fnAlpha50, run50: disposable50 };
    T50.run50();
    break;
  }
}
...
// of the malformed clause's own (unresolvable) name.
...
// 19. (ROUND 18) A getter property whose OWN body is `this`-free but whose
//     property NAME differs from any handler this literal actually resolves
//     via identifier-value/shorthand — proves the new getter-escapes-
//     unconditionally rule in `literalHasUnmodeledThisReference` is scoped
//     to `get`-flavoured `method_definition` nodes specifically, not to
//     every accessor or every method: an ordinary getter co-existing with
//     an ordinary identifier-valued handler must not make the WHOLE site
//     escape merely because condition 4 now treats ITS OWN sibling getter
//     with extra suspicion — each property is still judged on its own
//     shape. (This shape is necessarily paired with case (ax) below, which
//     shows the SAME construction's getter escaping when it is the one
//     under test; this shape isolates the claim that a getter's mere
//     PRESENCE does not retroactively taint an unrelated, already-safe
//     property in the same literal. The co-located setter's OWN body is
//     deliberately `this`-free here — a `this`-using setter body is a
//     GENUINE escape the existing, unchanged `subtreeContainsThisKeyword`
//     check already catches correctly, same as any other method, and
//     mixing that in would confound this shape's own, narrower claim.)
...
const T51 = {
  alpha: fnAlpha51,
  run51,
  get label51() { return 'unrelated'; },              // this-free getter, unrelated property
  set label51(v) { label51Store = v; },               // this-free setter body — genuinely safe
};
T51.run51();
// EXPECT: fnAlpha51 dead (nothing invokes `.alpha(`); escapes === 0 for
// T51's site — the co-located getter/setter do not perturb it.
...
//      global-object identifier itself — proves `isGlobalObjectQualifiedWrite`'s
//      `unwrapParens` routing (both arms, and its own call site) actually
//      closes the gap, not merely that a plausible one-line fix was
//      described: `object.type === 'identifier'` alone is false for
//      `(globalThis)`, a `parenthesized_expression`.
function fnAlpha55() { return 1; }
var run55 = () => {};                                        // module-level arrow — looks safe
(globalThis).run55 = function () { return this.alpha(); };   // PAREN-WRAPPED global-object write

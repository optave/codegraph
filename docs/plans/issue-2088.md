 ...
 *
 * The `node.type === 'function_declaration'` check above is an exact string
 * match against ONE of two sibling grammar kinds `FUNCTION_SCOPE_NODE_TYPES`
 * itself already distinguishes — a generator function DECLARATION
 * (`function* name(){}`) is `generator_function_declaration`, a separate
 * kind, and is therefore invisible to this count (fail-safe-already, not
 * confidently wrong, since nothing here ever returns such a node as
 * `result` either) — the SAME pre-existing gap `findTopLevelFunctionNodeByName`'s
 * own direct-children loop above already carries for this identical test,
 * one hoisting hop further rather than newly introduced here; filed
 * separately, not widened inline — #2636.
 *
...
function subtreeContainsReassignmentOf(node: TreeSitterNode, name: string, depth: number): boolean {
  ...
  if (node.type === 'assignment_expression' || node.type === 'augmented_assignment_expression') {
    ...
    const left = node.childForFieldName('left');
    if (left && patternBindsName(left, name) && !node.childForFieldName('kind')) return true;
  }
  for (let i = 0; i < node.childCount; i++) {
    const child = node.child(i);
...
Mirrors WU-2 one-for-one, including every round-7, round-8, round-9, round-10, round-11, round-12, round-13, AND round-14 refinement — dual-engine parity (ADR-001) means none of round 7's finding 1/2/3/4/5 fixes, round 8's finding 1/2/3 fixes, round 9's finding 1 fix, round 10's finding 1/2 fixes, round 11's finding 1/2 fixes, round 12's finding 1 fix, round 13's finding 1 fix, nor round 14's finding 1/2 fixes, are TS-only:
...
**The correlation test** covers the eight shapes the design claims (three from earlier rounds, two added in round 8 to close the testing blind spot described below, two added in round 9 for finding 1's over-escape check and finding 2's pass-ordering check, and one added in round 15 to prove the var/Annex-B hoist-count fix's own `let`/`const` exclusion), each asserted under **both** engines (`--engine wasm` and `--engine native`, skipped with an explicit message rather than silently if `isNativeAvailable()` is false — never a silent skip):
...
- [ ] **(round 14, #2088 finding 2 — the single-declaration contract; extended round 15, #2088 finding 1, to the module's own VAR SCOPE, not merely `root`'s direct children).** `findTopLevelFunctionNodeByName` never returns a top-level declaration of `name` with confidence when a SECOND (or later) declaration of the identical name also exists ANYWHERE in the module's own var scope — enforced structurally by counting every direct-child declaration encountered (a `function_declaration` or a `variable_declarator` match, regardless of its own value's shape) PLUS, as of round 15, every `var` declarator or block-level `function_declaration` reachable from the module root without crossing a function boundary (`countHoistedVarScopeDeclarations`, reusing `functionScopeDeclaresVar`'s own traversal rule) — and returning `null` once the combined total exceeds one, not merely by returning whichever direct-child declaration the scan happens to reach first. **Deliberately excludes a `lexical_declaration` (`let`/`const`) from the round-15 hoisted count**: that is a distinct, block-scoped binding, not a redeclaration, and is already the shadow axis's own concern (`findResolvingScopeNode`) — verified by WU-10's new correlation shape 8, which proves this exclusion holds rather than merely stating it. Verified by WU-10 escape-fallback cases (ah)/(ai) (a direct-child `var`/`function` redeclaration must escape, round 14) and, round 15, cases (aj) (a `var` hoisted through a bare `if` block must escape too) and (ak) (the identical Annex-B block-level-`function` shape must escape too) — none may silently resolve to a superseded declaration's own value, whether the redeclaration sits at module top level or is reachable only by hoisting through a nested block.

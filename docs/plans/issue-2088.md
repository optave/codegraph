 ...
 * > comment already gives for a BRACED `case`. But `for_statement`'s own
 * > case (`src/extractors/javascript.ts`, the case immediately preceding
 ...
 * > type over, and one this design had not yet named. A C-style for-loop's
 * > `using`-declared init clause is grammar-valid (verified runnable: `for
 * > (using x = mk(); false; ) {}` parses and runs under Node 22.18 with
 * > `--js-explicit-resource-management`) and shares the SAME "one clause,
 * > one shared scope, no wrapping block" shape `switch_body`'s own clauses
 * > do. Concretely, verified runnable: `function fnA() { return 1; }
 * > function run() { return 0; } function install() { for (using run =
 * > mk(); ; ) { const T = { alpha: fnA, run }; return T.run(); } }
 * > install();` — evaluates to `1`: inside the loop body, `run` resolves to
 * > the `using`-declared, `this`-using function, not the outer, `this`-free
 * > `function run()` declaration. Pre-round-17, `findResolvingScopeNode` has
 * > no `for_statement` disjunct of its own, and `introducesShadowedBinding`'s
 * > `for_statement` case does not recognise `using_declaration`, so the walk
 * > never notices this shadow and `run` resolves to the OUTER decoy with
 * > full confidence — the identical failure mode #2637 names, one node kind
 * > further. Fixed the same way: a further disjunct on `findResolvingScopeNode`
 * > alone, scanning the `for_statement`'s own direct children (never wrapped
 * > in a clause container the way `switch_body`'s are, so no nested loop is
 * > needed here) for a `using_declaration` matching via the same, unmodified
 * > `declarationDeclaresName`. `introducesShadowedBinding` is again left
 * > untouched, for the identical reason. This completes the audit of every
 * > `SCOPE_NODE_TYPES` member against this exact gap shape — no member
 * > remains unchecked. Also closes #2637 (the two gaps share one root cause
 * > and one issue). Matches escape-fallback case (av) and correlation shape
 * > 17.
 * >
...
function findResolvingScopeNode(node: TreeSitterNode, name: string): TreeSitterNode | undefined {
  ...
  while (current) {
    ...
    // ROUND 17 (#2088, #2637) — the `for_statement` counterpart of the
    // SAME missing-`using_declaration` gap, found while auditing every
    // `SCOPE_NODE_TYPES` case for it rather than stopping at `switch_body`
    // alone; see the essay above. Mirrors `introducesShadowedBinding`'s own
    // `for_statement` case's flat child-scan shape (a C-style for-loop's
    // init clause is an unwrapped direct child, unlike `switch_body`'s own
    // nested clause structure) — `lexical_declaration` only there, the
    // identical omission.
...
function isGlobalObjectQualifiedWrite(node: TreeSitterNode, name: string): boolean {
  if (node.type === 'member_expression') {
    ...
    return (
      ...
      GLOBAL_OBJECT_NAMES.has(object.text) &&
      !!property &&
      property.text === name
    );
  }
  ...
  if (node.type === 'subscript_expression') {
    const object = node.childForFieldName('object');
    if (!object || object.type !== 'identifier' || !GLOBAL_OBJECT_NAMES.has(object.text)) {
      return false;
    }
...
**The correlation test** covers the seventeen shapes the design claims (three from earlier rounds, two added in round 8 to close the testing blind spot described below, two added in round 9 for finding 1's over-escape check and finding 2's pass-ordering check, one added in round 15 to prove the var/Annex-B hoist-count fix's own `let`/`const` exclusion, four added in round 16 — one guard per that round's own four fixes, proving none of them over-escapes the legitimate neighbouring shape — and five added in round 17, the identical discipline applied to that round's own four fixes (the `switch_body` and `for_statement` halves of the `using_declaration`-shadow fix each get their own guard, and the redeclaration-count and write-scan halves of the `var`-kind for-of/for-in fix share one)), each asserted under **both** engines (`--engine wasm` and `--engine native`, skipped with an explicit message rather than silently if `isNativeAvailable()` is false — never a silent skip):
...
// (av) (ROUND 17, #2088, #2637) A `using`/Explicit Resource Management
//      declaration shadow in a C-STYLE for-loop's own init clause —
//      `introducesShadowedBinding`'s `for_statement` case recognises only
//      `lexical_declaration` among its direct children, the identical
//      omission its `switch_body` case had (case (ar)), found by auditing
//      every `SCOPE_NODE_TYPES` member against this exact gap shape rather
//      than stopping at `switch_body` alone. `run45`'s declaring scope for
//      condition 4's own question is the for-loop's own shared init/body
//      scope, but pre-round-17 `findResolvingScopeNode` never notices the
//      `using` declaration, the walk continues past the `for_statement` to
//      `root`, and `run45` resolves to the OUTER, `this`-free module-level
//      decoy with full confidence — the identical failure mode case (ar)
//      closed for a `switch_body` ancestor instead of a `for_statement`
//      one. Verified runnable under Node 22.18.0 with
//      `--js-explicit-resource-management`: `T45.run45()` evaluates to `1`.
//      Fixed in `findResolvingScopeNode` alone, NOT in
//      `introducesShadowedBinding` — see that function's own round-17
//      essay for why. No decoy needed: the disposable function's own
//      `this.alpha()` call populates T2 once the site correctly escapes,
//      the same way case (ar)'s does. Also closes #2637 (the two gaps
//      share one root cause and one issue).
